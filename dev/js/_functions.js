...
(function ($) {
	...
	$('#nav-button-label').on('click', function (event) {
		...
		setTimeout(function () {
			scrollIfNeeded(document.querySelector('.aside-mobile .current-menu-item'), document.querySelector('[off-canvas*=id-1]'));
		},400);
		setTimeout(function () {
			scrollIfNeeded(document.querySelector('.aside-mobile .m-open'), document.querySelector('[off-canvas*=id-1]'));
		},400);
	...
	$('[off-canvas] a:not(".navmenu-link-toggle, .category a"), .close-aside-mobile').on('click', function (event) {
		event.preventDefault();
		event.stopPropagation();
